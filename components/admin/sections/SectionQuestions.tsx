...
export function SectionQuestions({ data, setData }: SectionQuestionsProps) {
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        <h4 className="text-[10px] font-mono tracking-widest uppercase text-black/40">Knowledge Base (FAQ)</h4>
                        <p className="text-xs text-black/60">Manage community inquiries and answers</p>
                    </div>
                    <button 
                        onClick={addItem}
                    ...
                    {items.map((item: any) => (
                        ...
                        >
                            ...
                            <div className="flex-1 space-y-4">
                                <div className="space-y-2">
                                    <label className="block text-[10px] font-mono tracking-widest text-black/40 uppercase">Question</label>
                                    <input value={item.question} onChange={(e) => updateItem(item.id, "question", e.target.value)} className="w-full bg-white border border-black/10 px-4 py-3 text-sm outline-none focus:border-black" />
                                </div>
                                <div className="space-y-2">
                                    <label className="block text-[10px] font-mono tracking-widest text-black/40 uppercase">Detailed Answer</label>
                                    <AutoResizingTextarea value={item.answer} onChange={(val) => updateItem(item.id, "answer", val)} className="bg-white border border-black/10 px-4 py-3 text-sm outline-none focus:border-black" />
                                </div>
                            </div>
